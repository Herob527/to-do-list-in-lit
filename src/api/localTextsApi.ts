...
import type { Api, TextEntry } from "./type";
import { v4 } from "uuid";
...
const localTextsApi = () => {
  ...
  return {
    getAll: () =>
      JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]") as TextEntry[],

    addNewItem: () => {
      const dataItem = { id: v4(), value: "", category: "None" } satisfies TextEntry;
